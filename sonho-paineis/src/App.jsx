...
function exportXlsx(pedidos, kits, precos, label) {
  ...
  const kitIds=[...new Set(pedidos.flatMap(p=>getItens(p).map(it=>it.kitId)).filter(Boolean))];
  ...
  kitIds.forEach(id=>{
    // pedidos que contêm este kit (multi-itens)
    const pr=precos[id]||0;
    const pairsK=pedidos.flatMap(p=>getItens(p).filter(it=>it.kitId===id).map(it=>({qtd:it.qtd||0,fac:PF(p.pagamento)})));
    const qi=pairsK.reduce((s,x)=>s+x.qtd,0);
    const tot=F2(pairsK.reduce((s,x)=>s+pr*x.qtd,0));
    const rec=F2(pairsK.reduce((s,x)=>s+pr*x.qtd*x.fac,0));
    const its={length:pedidos.filter(p=>getItens(p).some(it=>it.kitId===id)).length};
  ...
  const tT =F2(pedidos.reduce((s,p)=>s+getItens(p).reduce((a,it)=>a+(precos[it.kitId]||0)*(it.qtd||0),0),0));
  const tTR=F2(pedidos.reduce((s,p)=>s+getItens(p).reduce((a,it)=>a+(precos[it.kitId]||0)*(it.qtd||0),0)*PF(p.pagamento),0));
  ...
  kitIds.forEach(id=>{
    ...
    const rows=[
      [nome],[`Período: ${label}`],
      [`Pedidos: ${its.length}   |   Itens: ${its.reduce((s,p)=>s+getItens(p).reduce((a,x)=>a+(x.qtd||0),0),0)}`],
      [],HDR,
      ...its.flatMap((p,i)=>mkRows(p,i)),
      [],
    ];
