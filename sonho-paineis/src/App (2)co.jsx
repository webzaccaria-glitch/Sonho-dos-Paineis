...
// ─────────────────────────────────────────────
const KITS_PADRAO = [
  { id:'kit01', nome:'Kit 01 — painel + suporte', metragem:+(10*.55/3).toFixed(4), suporte:15 },
  { id:'kit02', nome:'Kit 02 — 5 painéis',        metragem:+(5*.55/3).toFixed(4),  suporte:0  },
  { id:'kit03', nome:'Kit 03 — unitário',          metragem:+(1*.55/3).toFixed(4),  suporte:0  },
  { id:'kit04', nome:'Kit 04 — Painel redondo',    metragem:1.60,                   suporte:0  },
  { id:'kit05', nome:'Kit 05 — kit + portal',      metragem:6.00,                   suporte:0  },
  { id:'kit06', nome:'Kit 06 — Portal',            metragem:2.10,                   suporte:0  },
  { id:'kit07', nome:'Kit 07 — Shopee',            metragem:+(10*.55/3).toFixed(4), suporte:0  },
];
...
const PREM_PAD    = {tt:460,tr:80,pt:520,pr:300,it:1080,ir:1000,cal:3.5,et:18,er:100,lt:50,lr:500,sup:15};
...
// STORAGE
// ─────────────────────────────────────────────
const dbGet = async (key,fb) => { try{ const r=localStorage.getItem(key); return r?JSON.parse(r):fb; }catch{ return fb; } };
const dbSet = async (key,v)  => { try{ localStorage.setItem(key,JSON.stringify(v)); }catch{} };
...
export default function App() {
  ...
  const custosKits = useMemo(()=>{
    const cT=prem.tt/prem.tr, cP=prem.pt/prem.pr, cI=prem.it/prem.ir;
  ...
  const totais = useMemo(()=>{
    let prev=0,rec=0;
    pedidos.forEach(p=>{ const v=(precos[p.kitId]||0)*(p.qtd||0); prev+=v; rec+=v*pFactor(p); });
  ...
  const resumo = useMemo(()=>{
    const pk=kits.map(k=>{
      const its=pedidos.filter(p=>p.kitId===k.id);
      const qtd=its.reduce((s,p)=>s+(p.qtd||0),0);
      const pu=precos[k.id]||0, cu=custosKits.find(c=>c.id===k.id)?.det.tot||0;
      const rP=its.reduce((s,p)=>s+pu*(p.qtd||0),0);
      const rR=its.reduce((s,p)=>s+pu*(p.qtd||0)*pFactor(p),0);
  ...
  const unlock = async (senha)=>{
    if(!pwdHash){ const h=await sha256(senha); setPwd(h); setAdm(true); return {ok:true}; }
    const h=await sha256(senha);
    if(h===pwdHash){ setAdm(true); return {ok:true}; }
    ...
    <div className="min-h-screen" style={{background:'#FAF7F2'}}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 pb-24">
        ...
        {tab==='exportar' && isAdmin &&
          <TabExportar pedidos={pedidos} kits={kits} precos={precos}/>}
      </main>
...
function TabPedidos({kits,pedidos,setPedidos,precos,totais,isAdmin}) {
  ...
  const [novoKit,    setNK]=useState('');
  const [novaQtd,   setNQ]=useState('');
  const [novoPag,   setNP]=useState('integral');
  const [form,      setFm]=useState(false);
  const [filtro,    setFi]=useState('todos');

  const add = ()=>{
    if(!novoCliente.trim()||!novoKit||!(+novaQtd>0)) return;
    const id=(pedidos.length?Math.max(...pedidos.map(p=>p.id))+1:1);
    setPedidos(p=>[...p,{id,cliente:novoCliente.trim(),kitId:novoKit,qtd:+novaQtd,pagamento:novoPag,data:new Date().toISOString()}]);
    setNC('');setNK('');setNQ('');setNP('integral');setFm(false);
  };
          ...
          <div className="space-y-3">
            <Fld l="Cliente">
              <input type="text" value={novoCliente} onChange={e=>setNC(e.target.value)}
                ...
                className="inp w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:border-stone-400"/>
            </Fld>
            <Fld l="Kit">
              <Sel value={novoKit} onChange={e=>setNK(e.target.value)}>
                <option value="">Selecione…</option>
                {kits.map(k=><option key={k.id} value={k.id}>{k.nome} — {brl(precos[k.id]||0)}</option>)}
              </Sel>
            </Fld>
            <Fld l="Quantidade">
              <input type="number" inputMode="numeric" value={novaQtd} onChange={e=>setNQ(e.target.value)}
                placeholder="0"
            ...
            {novoKit&&+novaQtd>0&&(
              <div className="bg-stone-50 rounded-xl px-4 py-3 flex justify-between items-center">
                ...
                <span className="fd text-xl text-stone-900">{brl((precos[novoKit]||0)*+novaQtd)}</span>
              </div>
            )}
            <button onClick={add} disabled={!novoCliente.trim()||!novoKit||!(+novaQtd>0)}
      ...
      {lista.length===0 ? (
        ...
        <div className="space-y-2">
          {lista.map(p=>{
            ...
            const pr=precos[p.kitId]||0, tot=pr*(p.qtd||0);
              ...
              <div key={p.id} className="bg-white rounded-2xl border border-stone-200/80 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-stone-900 truncate">{p.cliente}</div>
                    <div className="text-xs text-stone-500 truncate">{kit?.nome||<em>kit removido</em>} · {p.qtd}un</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="fd text-base text-stone-900">{brl(tot)}</div>
                    <div className="text-[11px] text-stone-400">{brl(pr)} un</div>
                  </div>
            ...
            <div key={k.id} className="bg-white rounded-2xl p-5 border border-stone-200/80">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="text-xs text-stone-500 mt-0.5">{+k.metragem.toFixed(2)}m linear · extra {brl(k.suporte||0)}</div>
                </div>
            ...
            <div key={k.id} className="bg-white rounded-2xl border border-stone-200/80 overflow-hidden">
              <div className="p-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-stone-500 flex flex-wrap gap-x-2 mt-0.5">
...
function ModalKit({title,ini,onSave,onClose}) {
  ...
  const save=()=>{
    ...
    setEr(''); onSave({nome:nome.trim(),metragem:m,suporte:parseFloat(sup)||0,pr:parseFloat(pr)||0});
  };
...
function exportXlsx(pedidos, kits, precos, label) {
  ...
  const mkRow=(p,i)=>{
    const pr=precos[p.kitId]||0, tot=F2(pr*(p.qtd||0)), rec=F2(tot*PF(p.pagamento));
    return [i+1,p.id,DT(p.data),p.cliente,KN(p.kitId),p.qtd||0,F2(pr),tot,PL[p.pagamento||'integral'],rec,F2(tot-rec)];
  };
  ...
  const allRows=[['TODOS OS PEDIDOS — '+label],[],HDR,...pedidos.map((p,i)=>mkRow(p,i))];
  const tT=F2(pedidos.reduce((s,p)=>s+(precos[p.kitId]||0)*(p.qtd||0),0));
  const tTR=F2(pedidos.reduce((s,p)=>s+(precos[p.kitId]||0)*(p.qtd||0)*PF(p.pagamento),0));
...
function ModalAuth({hasHash,onClose,onSubmit}) {
  ...
  const go=async()=>{
    ...
    setBs(false);
    if(!r.ok){setEr(r.msg||'Erro');setPw('');setPw2('');}
  };
