...
import { Plus, Trash2, Package, Calculator, TrendingUp, X, Check, AlertCircle,
         ChevronDown, Lock, Unlock, Eye, EyeOff, KeyRound, Pencil, PackagePlus,
         FileDown, FileSpreadsheet, Calendar } from 'lucide-react';
...
const mesHj  = () => new Date().toISOString().slice(0,7);
const pFactor  = p   => PAG_CFG[p?.pagamento||'integral']?.factor??1;
...
export default function App() {
  ...
  const totais = useMemo(()=>{
    let prev=0,rec=0;
    pedidos.forEach(p=>{ const v=pedTotal(p,precos); prev+=v; rec+=v*pFactor(p); });
  ...
  const resumo = useMemo(()=>{
    const pk=kits.map(k=>{
      ...
      const pares=pedidos.flatMap(p=>getItens(p).filter(it=>it.kitId===k.id).map(it=>({qtd:it.qtd||0,fac:pFactor(p)})));
  ...
  return (
    <div className="min-h-screen" style={{background:'#FAF7F2'}}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 pb-24">
        ...
        {tab==='resumo' && isAdmin &&
          <TabResumo resumo={resumo} custosKits={custosKits}/>}
...
function TabPedidos({kits,pedidos,setPedidos,precos,totais,isAdmin}) {
  const [novoCliente, setNC] = useState('');
  ...
  const canSubmit  = novoCliente.trim() && novoItens.some(it=>it.kitId&&+it.qtd>0);

  const add = ()=>{
    if(!canSubmit) return;
    const validos = novoItens.filter(it=>it.kitId&&+it.qtd>0);
    const id = pedidos.length ? Math.max(...pedidos.map(p=>p.id))+1 : 1;
    setPedidos(p=>[...p,{id, cliente:novoCliente.trim(), itens:validos, pagamento:novoPag, data:new Date().toISOString()}]);
    setNC(''); setNI([{kitId:'',qtd:1}]); setNP('integral'); setFm(false);
  };

  const del  = id => setPedidos(p=>p.filter(x=>x.id!==id));
  const setPg= (id,v)=>setPedidos(p=>p.map(x=>x.id===id?{...x,pagamento:v}:x));

  const lista = useMemo(()=>{
    const sorted=[...pedidos].sort((a,b)=>new Date(b.data)-new Date(a.data));
    if(filtro==='todos') return sorted;
    return sorted.filter(p=>(p.pagamento||'integral')===filtro);
  },[pedidos,filtro]);

  const cnt=useMemo(()=>({
    todos:pedidos.length,
    pendente:pedidos.filter(p=>(p.pagamento||'integral')==='pendente').length,
    meio:pedidos.filter(p=>p.pagamento==='meio').length,
    integral:pedidos.filter(p=>(p.pagamento||'integral')==='integral').length,
  }),[pedidos]);

  return (
    <div className="space-y-3">
      {isAdmin && (
        <div className="grid grid-cols-3 gap-2 ai">
          <MiniCard l="Pedidos"   v={pedidos.length} num/>
          <MiniCard l="Recebido"  v={brl(totais.rec)}  cor="#3F6E3A"/>
          <MiniCard l="A receber" v={brl(totais.pend)}  cor={totais.pend>0?'#B5302B':'#78716C'}/>
        </div>
      )}
      ...
      {!form ? (
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200/80 ai">
          ...
          <div className="space-y-4">
            ...
            {/* Cliente */}
            <Fld l="Cliente">
              <input type="text" value={novoCliente} onChange={e=>setNC(e.target.value)}
                  ...
                  <div key={idx} className="flex gap-2 items-center bg-stone-50 rounded-xl p-2.5 border border-stone-200">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {it.kitId&&it.qtd>0&&(
                          <span className="text-xs text-stone-500 ml-auto">{brl((precos[it.kitId]||0)*it.qtd)}</span>
                        )}
                      </div>
                    </div>
                    {novoItens.length>1&&(
                      <button onClick={()=>removeItem(idx)}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-stone-300 hover:text-red-500 hover:bg-red-50 transition-colors flex-shrink-0">
                        <X size={14}/>
                      </button>
                    )}
                  </div>
      ...
      {lista.length===0 ? (
        <div className="text-center py-14">
          ...
          <p className="text-stone-400 text-sm">{pedidos.length===0?'Nenhum pedido ainda':'Nenhum pedido neste filtro'}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {lista.map(p=>{
            ...
            const pk=p.pagamento||'integral', ps=PAG_CFG[pk];
            return (
              <div key={p.id} className="bg-white rounded-2xl border border-stone-200/80 p-4">
                <div className="flex items-start gap-3">
                      ...
                      {itens.map((it,i)=>{
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between text-xs text-stone-500">
                            <span className="truncate mr-2">{kit?.nome||<em>kit removido</em>}</span>
                            <span className="flex-shrink-0 tabular-nums">{it.qtd}un × {brl(pr)}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <button onClick={()=>del(p.id)}
                    className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-stone-300 hover:text-red-500 hover:bg-red-50 transition-colors">
                    <Trash2 size={15}/>
                  </button>
                </div>
                {/* Pagamento */}
                <div className="mt-3 pt-3 border-t border-stone-100 flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-1">
                    {PAG_KEYS.map(k=>{ const s=PAG_CFG[k]; const sel=pk===k; return (
                ...
                {pk!=='integral'&&(
                  <div className="mt-1.5 text-xs text-right text-stone-500 flex gap-3 justify-end">
                    <span>Recebido: <b className="text-stone-700">{brl(tot*ps.factor)}</b></span>
                    <span>A receber: <b style={{color:'#B5302B'}}>{brl(tot*(1-ps.factor))}</b></span>
                  </div>
                )}
              </div>
            );
...
// TAB RESUMO (admin)
// ─────────────────────────────────────────────
function TabResumo({resumo,custosKits}) {
...
function TabExportar({pedidos,kits,precos}) {
  ...
  const filtrados = useMemo(()=>{
    ...
    if(tipo==='diario')  return sorted.filter(p=>p.data.slice(0,10)===dataDia);
    if(tipo==='semanal'){ const{s,e}=semanaRange(dataSem); return sorted.filter(p=>{const d=new Date(p.data);return d>=s&&d<=e;}); }
    if(tipo==='mensal')  return sorted.filter(p=>p.data.slice(0,7)===dataMes);
  ...
  const labelPeriodo = useMemo(()=>{
    ...
    if(tipo==='mensal'){
      ...
      const nm=new Date(+y,+m-1,1).toLocaleString('pt-BR',{month:'long'});
  ...
  // ── Resumo por kit para pré-visualização ──
  const preview = useMemo(()=>{
    const ids=[...new Set(filtrados.map(p=>p.kitId))];
    return ids.map(id=>({
      id,
      nome: kits.find(k=>k.id===id)?.nome||`ID:${id}`,
      peds: filtrados.filter(p=>p.kitId===id).length,
      qtd:  filtrados.filter(p=>p.kitId===id).reduce((s,p)=>s+(p.qtd||0),0),
      tot:  filtrados.filter(p=>p.kitId===id).reduce((s,p)=>s+(precos[p.kitId]||0)*(p.qtd||0),0),
    }));
          ...
          : <>
              ...
              <div className="px-5 py-3 border-t border-stone-100 flex justify-between items-center bg-stone-50/50">
                ...
                <span className="fd text-lg font-semibold text-stone-900">
                  {brl(filtrados.reduce((s,p)=>s+(precos[p.kitId]||0)*(p.qtd||0),0))}
                </span>
              </div>
            </>
        }
...
function exportXlsx(pedidos, kits, precos, label) {
  ...
  const DTS = iso => new Date(iso+'T12:00:00').toLocaleDateString('pt-BR');
  ...
  // Gera uma linha por item do pedido
  const mkRows=(p,startIdx)=>{
    const its = p.itens || (p.kitId ? [{kitId:p.kitId,qtd:p.qtd||1}] : []);
    const totPed = F2(its.reduce((s,it)=>s+(precos[it.kitId]||0)*(it.qtd||0),0));
    const recPed = F2(totPed*PF(p.pagamento));
    return its.map((it,j)=>{
      const pr=precos[it.kitId]||0, tot=F2(pr*(it.qtd||0)), rec=F2(tot*PF(p.pagamento));
      return [startIdx+1, p.id, j===0?DT(p.data):'', j===0?p.cliente:'', KN(it.kitId),
              it.qtd||0, F2(pr), tot, j===0?PL[p.pagamento||'integral']:'', rec, F2(tot-rec)];
    });
  };

  const kitIds=[...new Set(pedidos.map(p=>p.kitId))];

  // ── Aba 1: Resumo Geral ──
  let tPed=0,tItens=0,tRec=0,tTot=0;
  const rRows=[
  ...
  kitIds.forEach(id=>{
    const its=pedidos.filter(p=>p.kitId===id);
    const pr=precos[id]||0;
    const qi=its.reduce((s,p)=>s+(p.qtd||0),0);
    const tot=F2(its.reduce((s,p)=>s+pr*(p.qtd||0),0));
    const rec=F2(its.reduce((s,p)=>s+pr*(p.qtd||0)*PF(p.pagamento),0));
    rRows.push([KN(id),its.length,qi,tot,rec,F2(tot-rec)]);
    tPed+=its.length;tItens+=qi;tTot+=tot;tRec+=rec;
  });
  ...
  const allRows=[['TODOS OS PEDIDOS — '+label],[],HDR,...pedidos.flatMap((p,i)=>mkRows(p,i))];
  const tT=F2(pedidos.reduce((s,p)=>s+(p.itens||[{kitId:p.kitId,qtd:p.qtd||0}]).reduce((a,it)=>a+(precos[it.kitId]||0)*(it.qtd||0),0),0));
  const tTR=F2(pedidos.reduce((s,p)=>s+(p.itens||[{kitId:p.kitId,qtd:p.qtd||0}]).reduce((a,it)=>a+(precos[it.kitId]||0)*(it.qtd||0),0),0)*PF(p.pagamento));
  allRows.push([],[,'','','TOTAIS:',pedidos.reduce((s,p)=>s+(p.qtd||0),0),'',tT,'',tTR,F2(tT-tTR)]);
  ...
  // ── Aba por kit ──
  kitIds.forEach(id=>{
    const nome=KN(id), its=pedidos.filter(p=>p.kitId===id);
    if(!its.length) return;
    const pr=precos[id]||0;
    const rows=[
      [nome],[`Período: ${label}`],
      [`Pedidos: ${its.length}   |   Itens: ${its.reduce((s,p)=>s+(p.qtd||0),0)}`],
      [],HDR,
      ...its.map((p,i)=>mkRow(p,i)),
      [],
    ];
    // resumo por status de pagamento
    rows.push(['— Resumo por pagamento —']);
    [['integral','Pagos'],['meio','50% pagos'],['pendente','Pendentes']].forEach(([k,l])=>{
      const gs=its.filter(p=>(p.pagamento||'integral')===k);
      if(!gs.length) return;
      const t=F2(gs.reduce((s,p)=>s+pr*(p.qtd||0),0));
      const r=F2(gs.reduce((s,p)=>s+pr*(p.qtd||0)*PF(p.pagamento),0));
      rows.push([l,`${gs.length} pedido(s)`,gs.reduce((s,p)=>s+(p.qtd||0),0),t,r,F2(t-r)]);
    });
    const tK=F2(its.reduce((s,p)=>s+pr*(p.qtd||0),0));
    const rK=F2(its.reduce((s,p)=>s+pr*(p.qtd||0)*PF(p.pagamento),0));
    rows.push([],['TOTAL DO KIT','',its.reduce((s,p)=>s+(p.qtd||0),0),'',tK,'',rK,'',F2(tK-rK)]);
