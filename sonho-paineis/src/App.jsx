              ...
              <div>
                ...
                <div className="space-y-3">
                  {novoItens.map((it,idx)=>(
                    <div key={idx} className="bg-white rounded-2xl border-2 border-stone-100 overflow-hidden">
                      {/* Cabeçalho do item */}
                      <div className="flex items-center justify-between px-3.5 pt-3 pb-2">
                        <span className="text-[11px] font-bold uppercase tracking-wider text-stone-400">
                          {it.kitId ? '✓ Selecionado' : `Item ${idx+1} — escolha o kit`}
                        </span>
                        {novoItens.length>1&&(
                          <button onClick={()=>removeItem(idx)} className="text-stone-300 hover:text-red-500 transition-colors"><X size={14}/></button>
                        )}
                      </div>
                      {/* Lista visual de kits */}
                      <div className="px-3.5 pb-2 space-y-1.5 max-h-52 overflow-y-auto">
                        {kits.map(k=>{
                          const sel=it.kitId===k.id;
                          return (
                            <button key={k.id} onClick={()=>updateItem(idx,'kitId',k.id)}
                              className={`w-full flex items-center justify-between px-3.5 py-3 rounded-xl border-2 transition-all text-left gap-2 ${sel?'border-transparent':'border-stone-100 bg-stone-50 hover:border-stone-300'}`}
                              style={sel?{background:'#C65D3C',borderColor:'#C65D3C',color:'white'}:{}}>
                              <span className="text-sm font-medium leading-tight">{k.nome}</span>
                              <span className={`text-sm font-bold tabular-nums flex-shrink-0 ${sel?'opacity-80':'text-stone-500'}`}>{brl(precos[k.id]||0)}</span>
                            </button>
                          );
                        })}
                      </div>
                      {/* Quantidade + Total (só aparece após selecionar) */}
                      {it.kitId&&(
                        <div className="flex items-center gap-3 px-3.5 py-2.5 border-t border-stone-100 bg-stone-50">
                          <span className="text-xs text-stone-500 font-medium">Quantidade:</span>
                          <div className="flex items-center gap-1.5">
                            <button onClick={()=>updateItem(idx,'qtd',Math.max(1,(it.qtd||1)-1))}
                              className="w-7 h-7 rounded-lg bg-white border border-stone-200 flex items-center justify-center text-stone-500 hover:bg-stone-100 font-bold text-sm">−</button>
                            <input type="number" inputMode="numeric" value={it.qtd}
                              onChange={e=>updateItem(idx,'qtd',+e.target.value||1)}
                              className="w-12 bg-white border border-stone-200 rounded-lg py-1 text-sm text-stone-900 focus:outline-none focus:border-stone-400 text-center font-bold"/>
                            <button onClick={()=>updateItem(idx,'qtd',(it.qtd||1)+1)}
                              className="w-7 h-7 rounded-lg bg-white border border-stone-200 flex items-center justify-center text-stone-500 hover:bg-stone-100 font-bold text-sm">+</button>
                          </div>
                          <span className="ml-auto text-sm font-bold tabular-nums" style={{color:'#C65D3C'}}>
                            {brl((precos[it.kitId]||0)*(it.qtd||1))}
                          </span>
                        </div>
                      )}
                    </div>
...
function TabCustos({kits,custosKits,prem,setPrem,consumo,setCons,precos,setPrecos,pedidos,onAdd,onUpdate,onRemove,onMove,onChangePwd}) {
  ...
  return (
    <div>
      ...
      {sec==='itens'&&<GerenciarItens kits={kits} precos={precos} pedidos={pedidos} onAdd={onAdd} onUpdate={onUpdate} onRemove={onRemove} onMove={onMove}/>}
...
// GERENCIAR ITENS
// ─────────────────────────────────────────────
function GerenciarItens({kits,precos,pedidos,onAdd,onUpdate,onRemove,onMove}) {
              ...
              <div className="p-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-stone-500 flex flex-wrap gap-x-2 mt-0.5 items-center">
                    ...
                    <span>·</span>
                    <span className="px-1.5 py-0.5 rounded-full text-[10px] font-medium" style={{background:(k.tecido||'helanca')==='microfibra'?'#E0F0FF':(k.tecido||'helanca')==='helanca_tubular'?'#E0F5E0':(k.tecido||'helanca')==='helanca_ramada'?'#FFF3E0':'#F5EBE5',color:(k.tecido||'helanca')==='microfibra'?'#1E5A8A':(k.tecido||'helanca')==='helanca_tubular'?'#1E6A1E':(k.tecido||'helanca')==='helanca_ramada'?'#8A5A1E':'#A04A2E'}}>
                      {(k.tecido||'helanca')==='microfibra'?'Microfibra':
                      (k.tecido||'helanca')==='helanca_tubular'?'Tubular 1,20m':
                      (k.tecido||'helanca')==='helanca_ramada'?'Ramada 1,80m':
                      'Helanca'}
                    </span>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <div className="flex flex-col mr-0.5">
                    <button onClick={()=>onMove(k.id,'up')}
                      className="w-6 h-5 flex items-center justify-center text-stone-300 hover:text-stone-700 transition-colors" title="Subir"><ArrowUp size={11}/></button>
                    <button onClick={()=>onMove(k.id,'down')}
                      className="w-6 h-5 flex items-center justify-center text-stone-300 hover:text-stone-700 transition-colors" title="Descer"><ArrowDown size={11}/></button>
                  </div>
